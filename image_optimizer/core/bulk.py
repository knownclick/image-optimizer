...
from image_optimizer.core.utils import get_image_files, sanitize_filename
from image_optimizer.core.validation import validate_format, validate_rename_pattern
...
def _is_skipped(op_result: OperationResult) -> bool:
    """Check if an operation result was a skip (file already existed)."""
    return op_result.success and any("skip" in w.lower() for w in op_result.warnings)
...
def bulk_process(
    ...
    resize_percentage: float | None = None,
    ...
    """Process all images: optionally convert, resize, crop, compress, and write metadata.
    ...
    - resize_percentage: scale all images by this percentage.
    - crop_aspect_ratio: aspect ratio preset (e.g. "16:9").
    ...
    do_resize = resize_width is not None or resize_height is not None or resize_percentage is not None
    ...
    for i, fpath in enumerate(files):
        ...
        try:
            ...
            if crop_aspect_ratio:
                pipe.crop(aspect_ratio=crop_aspect_ratio, anchor=crop_anchor)
...
def bulk_rename(
    ...
    for i, (old, temp, final) in enumerate(temp_renames):
        if cancel_check and cancel_check():
            # Restore remaining temp files to originals
            for _, t, _ in temp_renames[i:]:
                orig = [o for o, tp, _ in temp_renames if tp == t]
                if orig:
                    try:
                        t.rename(orig[0])
                    except Exception:
