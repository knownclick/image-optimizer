...
from image_optimizer.core.validation import validate_output_path
...
def load_image(
    ...
    if real_format and ext_lower:
        from image_optimizer.core.constants import EXTENSION_TO_FORMAT
...
def save_image(
    ...
    if quality is not None:
        if fmt == ImageFormat.PNG:
            kwargs["compress_level"] = min(9, max(0, quality))
        else:
